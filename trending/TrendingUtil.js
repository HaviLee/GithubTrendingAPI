...
export default class TrendingUtil {
    static htmlToRepo(responseData) {
        ...
        var repos = [];
        var splitWithH3 = responseData.split('<article class="Box-row">');
        // splitWithH3.shift();
    ...
    static parseRepoBaseInfo(repo, htmlBaseInfo) {
        ...
        repo.url = url;
        repo.fullName = this.parseContentWithNote(htmlBaseInfo, '<span class="text-normal">', '</a>  </h1>').replace('</span>', '').replace(/\s/g,"");
    ...
    static parseRepoContributors(repo, htmlContributors) {
        ...
        for (var i = 0; i < splitWitSemicolon.length; i++) {
            ...
            if (url.length > 0) {
                let a = this.parseContentWithNote(url, 'src="', '" width="20');
